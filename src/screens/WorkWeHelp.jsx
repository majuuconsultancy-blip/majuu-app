...
function ServiceIcon({ tag, title }) {
  ...
  if (tag === "CV") return <AppIcon size={ICON_SM} icon={FileText} />;
  if (tag === "Jobs") return <AppIcon size={ICON_SM} icon={Target} />;
  if (tag === "Interview") return <AppIcon size={ICON_SM} icon={MessageSquareQuote} />;
...
export default function WorkWeHelp() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        console.error("WorkWeHelp getUserState error:", e);
  ...
  const singleServices = useMemo(() => listSingleRequestCatalogByTrack("work"), []);
        ...
        <div className="mt-6">
          <div className="flex items-end justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-300">
                Requests configured in SACC for Work {country}. Built-in single packages stay
                below.
              </p>
            </div>
